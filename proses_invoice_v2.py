...
def kloning_dan_ganti_file(source_folder, encrypted_folder):
    ...
    for root, dirs, files in os.walk(source_folder):
        ...
        masking_root = os.path.join(masking_folder, rel_path)
        os.makedirs(target_root, exist_ok=True)

        for file in files:
            src_file = os.path.join(root, file)

            # Cek apakah ini file ATM
            if "002-Invoice-Data-ATM Platform" in file and file.endswith(".txt"):
                os.makedirs(masking_root, exist_ok=True)
                shutil.copy2(src_file, os.path.join(masking_root, file))
                print(f"➡️ Dipindah ke Masking: {src_file}")
            else:
                shutil.copy2(src_file, os.path.join(target_root, file))

    print(f"\n✅ Folder dikloning ke: {done_folder}")
    print(f"✅ File ATM disendirikan ke: {masking_folder}")
